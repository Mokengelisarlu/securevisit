...
> **Status:** APPROVED — 2026-08-27 · **Phase 1 (Domain Foundation) + Phase 2 (Operator Mobile workflow) + Phase 3 (Host Portal, web) + Phase 4 (Notifications) COMPLETE** (schema deltas + migration `0022_*` + lifecycle server fns + API routes + kiosk waiting/expected/group operator screens + web host portal under `/hote/visits/*` with dashboard, pending approvals, approve/reject/postpone, upcoming, currently inside, history, pre-registration, notifications, profile; admin `visiteurs` enhancements: pending tab + waiting banner + full status badges + audit timeline & participants on visit detail; notifications emitted from every lifecycle mutation — create request, add participant, group/bulk check-in, per-member NO_SHOW, approve/reject/postpone/cancel, check-in/check-out — via a shared `notifyOperators` helper, list/read API + bell UI + dashboard unread stat using a SQL `COUNT`; `tsc`/lint/build green). **Phase 5 (Advanced) deferred.**
